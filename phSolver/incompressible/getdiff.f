...
c-----------------------------------------------------------------------
      ...
      real*8  yl(bsz,nshl,ndof), rmu(npro), xmudmi(npro,ngauss),
     &        shape(npro,nshl),   rho(npro),
     &        dwl(bsz,nshl),     sclr(npro),
     &        xl(bsz,nenl,nsd)
      integer n, e
...
c
      ...
      include "common.h"
      real*8 xmut(npro),xl(bsz,nenl,nsd),xmudmi(npro,ngauss)
 ...
 !this was made messy by the addEddyVisc routines  Must clean up later.
      ...
      real*8   yl(bsz,nshl,ndof), dwl(bsz,nshl), shape(npro,nshl)
      integer n, e
      rho(:)  = datmat(1,1,1)	! single fluid model, i.e., only 1 density
