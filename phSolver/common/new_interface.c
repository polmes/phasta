...
#ifdef intel
#include <winsock2.h>
#define Write_Velbar  WRITE_VELBAR
#define Write_VelbarS WRITE_VELBARS
#define Write_VelbarU WRITE_VELBARU
#define Read_VelbarS  READ_VELBARS
#else
#include <unistd.h>
#include <strings.h>
#if (!defined IBM )
#define Write_VelbarS write_velbars_
#define Write_Velbar  write_velbar_
#define Write_VelbarU write_velbaru_
#define Read_VelbarS  read_velbars_
#else
#define Write_Velbar  write_velbar
#define Write_VelbarS write_velbars
#define Write_VelbarU write_velbaru
...
Write_Velbar(  int* pid, 
    ...
    iarray[ 1 ] = (*numVars);
    iarray[ 2 ] = (*stepno);
    writeheader_( &irstou, "velbar ", 
                  (void*)iarray, &nitems, &isize, "double", "binary" );
    
        
    nitems = (*nshg)*(*numVars);    
    writedatablock_( &irstou, "velbar ",
...
Write_VelbarS(  int* pid, 
    ...
    iarray[ 1 ] = (*numVars);
    iarray[ 2 ] = (*stepno);
    writeheader_( &irstou, "velbar nfath ", 
                  (void*)iarray, &nitems, &isize, "double", "binary" );
    
        
    nitems = (*nshg)*(*numVars);
    writedatablock_( &irstou, "velbar nfath ",
                     (void*)(array1), &nitems, "double", "binary" );
...
Read_VelbarS(  int* pid, 
    ...
    int irstou;
    time_t timenow = time ( &timenow);
    int isize, nitems, nshg, numVars;
    int iarray[10];

    sprintf(rfile,"restart.%d.%d",*stepno,*pid+1);
    openfile_(rfile,"read", &irstou);
    
    nitems = 3;
    iarray[0] = -1;
    readheader_( &irstou, "velbar nfath", 
                  (void*)iarray, &nitems, "double", "binary" );
    if(iarray[0]==-1){
           (*ifail)=1;
           return;
    }
    ...
    numVars  = iarray[1];    
    nitems   = nshg*numVars;
    //printf ("read_velbarS: %i %i %i \n", *pid, nshg, numVars); 
    readdatablock_( &irstou, "velbar nfath",
                     (void*)(array1), &nitems, "double", "binary" );
   

    closefile_( &irstou, "read" );
...
Write_VelbarU(  int* pid, 
    ...
    iarray[ 1 ] = (*numVars);
    iarray[ 2 ] = (*stepno);
    writeheader_( &irstou, "velbar unstructured at recycle plane ", 
                  (void*)iarray, &nitems, &isize, "double", "binary" );
    
        
    nitems = (*nshg)*(*numVars);
    writedatablock_( &irstou, "velbar unstructured at recycle plane ",
                     (void*)(array1), &nitems, "double", "binary" );
        
    nitems = 3;
    writeheader_( &irstou, "velbar unstructured at inlet plane ", 
                  (void*)iarray, &nitems, &isize, "double", "binary" );
    
        
    nitems = (*nshg)*(*numVars);
    writedatablock_( &irstou, "velbar unstructured at inlet plane ",
